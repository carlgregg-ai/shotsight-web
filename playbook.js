...
function setup(){
  ...
  }).catch(err=>{root.removeAttribute('aria-busy');root.innerHTML=`<div class="playbook-error" role="alert"><strong>Playbook unavailable</strong><span>${esc(err.message)}</span><button class="secondary-button" id="playbookRetry">Try again</button></div>`;$('#playbookRetry')?.addEventListener('click',()=>{state.ready=false;setup()})});
}
function bindSearch(){
 const input=$('#playbookSearch'); if(!input||input.dataset.bound)return;input.dataset.bound='1';
 input.addEventListener('input',()=>{
   const q=input.value.trim().toLowerCase();state.intent=q?detectIntent(q):null;
   if(!q){state.filtered=[...state.lessons]}else{
     const ts=terms(q);state.filtered=state.lessons.map(l=>({l,score:scoreLesson(l,ts,q)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score).map(x=>x.l)
   }
   renderList(q)
 });
}
...
function scoreLesson(l,ts,q){
 const h=haystack(l);let score=0;
 if(h.includes(q))score+=20;
 for(const t of ts){if(h.includes(t))score+=3}
 const aliases=(l.aliases||[]).map(a=>a.toLowerCase());if(aliases.some(a=>q.includes(a)||a.includes(q)))score+=10;
 if(state.intent==='diagnose'&&(l.diagnose||[]).some(d=>ts.some(t=>haystack({name:'',aliases:[],understand:'',coaching:[],diagnose:[d]}).includes(t))))score+=5;
 return score;
}
function renderList(q=''){
 const list=$('#playbookList'); if(!list)return;
 const count=$('#playbookCount');if(count)count.textContent=q?`${state.filtered.length} ${state.intent==='diagnose'?'diagnostic':'lesson'} matches`:`${state.filtered.length} certified lessons`;
 ...
 list.querySelectorAll('[data-playbook-id]').forEach(b=>b.addEventListener('click',()=>openLesson(b.dataset.playbookId,b.dataset.focus)));
}
...
function renderLesson(l,focus='learn'){
 $('#playbookLessonTitle').textContent=l.name;const body=$('#playbookLessonBody');const diag=(l.diagnose||[])[0];const diagBlock=diagnosisHtml(diag);
 const learn=`<section class="pb-section"><span class="pb-kicker">WHAT YOU'RE SEEING</span><p>${esc(l.understand)}</p></section><section class="pb-section"><span class="pb-kicker">COACHING OPTIONS</span>${(l.coaching||[]).map(c=>`<article class="pb-claim"><span class="evidence-pill ${labelClass(c.label)}">${esc(c.label)}</span><p>${esc(c.text)}</p>${c.attribution?`<small>${esc(c.attribution)}</small>`:''}</article>`).join('')}</section>${l.holds?.length?`<section class="pb-section pb-hold"><span class="pb-kicker">DO NOT OVERCLAIM</span>${l.holds.map(h=>`<p>${esc(h)}</p>`).join('')}</section>`:''}`;
 const rest=`<section class="pb-section"><span class="pb-kicker">VISUAL LESSON SPEC</span><p>${esc(l.visual_spec?.target||'')}</p><ul>${(l.visual_spec?.show||[]).map(x=>`<li>${esc(x)}</li>`).join('')}</ul>${l.visual_spec?.warning?`<div class="pb-warning">${esc(l.visual_spec.warning)}</div>`:''}</section><section class="pb-section"><span class="pb-kicker">PRACTICE</span>${(l.drills||[]).map(d=>`<div class="pb-drill"><strong>${esc(d.name)}</strong><span class="evidence-pill ${labelClass(d.label)}">${esc(d.label)}</span></div>`).join('')}</section>`;
 body.innerHTML=focus==='diagnose'?diagBlock+learn+rest:learn+diagBlock+rest;
}
...
window.ShotSightPlaybook={init,openLesson};
