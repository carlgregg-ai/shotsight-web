...
export function createShooterStandPrior({
  expectedDirection='UNKNOWN',
  demonstratedFamilyProb={CROSSER:0.7,QUARTERER:0.2,LOOPER:0.05,OTHER:0.05},
  expectedPresentationDuration_s=0.95,
  demonstratedAngularSpan_rad=0.60,
  pickupProgressPrior=0.08,
  breakWindow={start:0.70,end:0.90},
  source='PRIOR_DEMONSTRATION_OR_STAND_KNOWLEDGE'
}={}){
...
export function buildShooterVisiblePresentationContext(observationHistory,belief,standPrior){
  ...
  if(!standPrior||standPrior.schema!=='SHOOTER_STAND_PRIOR_V1')throw new Error('SHOOTER_STAND_PRIOR_V1 required');
  ...
  const visible=observationHistory.filter(o=>o.visible&&Number.isFinite(o.az_rad));
  if(!visible.length)throw new Error('visible observations required');
  const first=visible[0],last=visible.at(-1);
  // Progress is inferred from how much angular travel has actually been seen relative to a coarse
  // demonstrated span. It is intentionally NOT computed from current target XYZ/future trajectory.
  const observedAngularTravel_rad=Math.abs(last.az_rad-first.az_rad);
  ...
  const remainingToBreakStart=clamp(standPrior.breakWindow.start-elapsedPresentationProgress,0,1);
  const remainingToBreakEnd=clamp(standPrior.breakWindow.end-elapsedPresentationProgress,0,1);
  const currentTime_s=last.observationTime_s;
  const estimatedRemainingOpportunity_s=remainingToBreakEnd*standPrior.expectedPresentationDuration_s;
  const context=freezePlain({
    schema:'SHOOTER_VISIBLE_PRESENTATION_CONTEXT_V1',
    source:'STAND_PRIOR_PLUS_OBSERVED_ANGULAR_TRAVEL',
    expectedDirection:standPrior.expectedDirection,
    demonstratedFamilyProb:standPrior.demonstratedFamilyProb,
    expectedPresentationDuration_s:standPrior.expectedPresentationDuration_s,
    demonstratedAngularSpan_rad:standPrior.demonstratedAngularSpan_rad,
    pickupProgressPrior:standPrior.pickupProgressPrior,
    breakWindow:standPrior.breakWindow,
    currentTime_s,
    elapsedPresentationProgress,
    remainingToBreakStart,
    remainingToBreakEnd,
    estimatedRemainingOpportunity_s,
    beliefConfidence:clamp(belief.confidence??0,0,1),
    familyProb:belief.familyProb,
    runtimeInputs:['SHOOTER_OBSERVATION_V1_HISTORY','MULTIFAMILY_BELIEF_V1','SHOOTER_STAND_PRIOR_V1'],
    forbiddenInterpretation:'REMAINING OPPORTUNITY IS AN OBSERVATION/DEMONSTRATION ESTIMATE, NOT ORACLE TIME-TO-INTERCEPT'
  });
...
export function fitWaitInformationModel(calibrationRows){
  ...
  const waits=[0.05,0.10,0.15],bins=[[0,0.2],[0.2,0.4],[0.4,0.6],[0.6,0.8],[0.8,1.000001]];
  const table={};
  for(const wait of waits){
    ...
    for(let bi=0;bi<bins.length;bi++){
      ...
      table[wait][bi]=xs.length?xs.reduce((a,r)=>a+Math.max(0,r.laterConfidence-r.initialConfidence),0)/xs.length:0;
    }
  }
  return freezePlain({schema:'WAIT_INFORMATION_MODEL_V1',confidenceBins:bins,expectedConfidenceGain:table,fitBoundary:'CALIBRATION OBSERVATIONS ONLY; NO HELDOUT OUTCOME OR ORACLE ACTION'});
}
...
export function chooseRunwayAwareObservationWait(context,belief,waitModel,{informationWeight=1,runwayPenalty=0.22,maxWait_s=0.15}={}){
  if(!context||context.schema!=='SHOOTER_VISIBLE_PRESENTATION_CONTEXT_V1')throw new Error('presentation context required');
  if(!belief||belief.schema!=='MULTIFAMILY_BELIEF_V1')throw new Error('belief required');
  if(!waitModel||waitModel.schema!=='WAIT_INFORMATION_MODEL_V1')throw new Error('wait information model required');
  ...
  const scored=candidates.map(wait_s=>{
    if(wait_s===0)return {wait_s,expectedInfoGain:0,opportunityCost:0,utility:0};
    const expectedInfoGain=waitModel.expectedConfidenceGain[wait_s]?.[bi]??0;
    const progressCost=wait_s/context.expectedPresentationDuration_s;
    const runway=Math.max(0.025,context.remainingToBreakEnd);
    const opportunityCost=runwayPenalty*progressCost/runway;
    return {wait_s,expectedInfoGain,opportunityCost,utility:informationWeight*expectedInfoGain-opportunityCost};
  });
  scored.sort((a,b)=>b.utility-a.utility||a.wait_s-b.wait_s);const best=scored[0];
  const out=freezePlain({schema:'RUNWAY_AWARE_WAIT_DECISION_V1',action:best.wait_s>0?'WAIT':'COMMIT',wait_s:best.wait_s,expectedInfoGain:best.expectedInfoGain,opportunityCost:best.opportunityCost,utility:best.utility,remainingToBreakEnd:context.remainingToBreakEnd,beliefConfidence:c,candidates:scored,runtimeInputs:['SHOOTER_VISIBLE_PRESENTATION_CONTEXT_V1','MULTIFAMILY_BELIEF_V1','WAIT_INFORMATION_MODEL_V1']});
