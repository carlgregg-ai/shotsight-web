...
import {
  ...
  quatMul, quatNorm, quatDerivativeBody,
  zeroAcceleration, gravityAcceleration,
...
// Gravity-only case should match closed form (constant acceleration RK4 is effectively exact apart from FP roundoff).
...
// Deterministic replay must be byte-for-byte JSON-equivalent for same inputs/time policy.
...
// Final shortened step must land exactly at requested tEnd, not overshoot.
...
// Simple nonlinear toy convergence test (not clay physics): dv/dt=-k v.
...
console.log(JSON.stringify({
  ...
  tests:{
    ...
    gravityAnalytic:true,deterministicReplay:true,finalStep:true,inputSafety:true,
    realisticClayFailClosed:true,rk4Convergence:true
  }
},null,2));
