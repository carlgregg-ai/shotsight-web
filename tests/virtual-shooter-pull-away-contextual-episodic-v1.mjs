...
const result=runL3PullAwayContextualEpisodicMemoryV1({nCrossersPerPartition:2,adaptiveEpisodes:20,evaluationRepetitions:4});
assert.equal(result.scoreBoundary,'ENGINEERING_CENTRELINE_DISC_PROXY_NOT_REAL_BREAK_PROBABILITY');
assert.equal(result.training.blockedAttempts,122);assert.equal(result.training.adaptiveEpisodes,20);assert.ok(result.training.contextualMemory.contextCount>=1);
assert.equal(result.calibration.attempts,8);assert.equal(result.heldout.attempts,8);
