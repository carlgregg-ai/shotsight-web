...
export const P8B_DEFAULT_CAMERA_R_CW=Object.freeze([Object.freeze([1,0,0]),Object.freeze([0,0,-1]),Object.freeze([0,1,0])]);
function gravityVector(g){return [0,0,-g];}

export function createCanonicalLooperGravityScenario({targetInitial_W=[-5,20,1.5],targetInitialVelocity_W=[10,4,12],shotOrigin_W=[0,0,1.5],cameraOrigin_W=[0,0,1.65],R_CW=P8B_DEFAULT_CAMERA_R_CW,shotTime_s=1.0,gravity_mps2=P8B_STANDARD_GRAVITY_MPS2,provider=constantSpeedTestProvider(400)}={}){
  ...
  return Object.freeze({id:'P8B_LOOPER_TOY_GRAVITY_ONLY_V1',status:'ENGINEERING_PROOF_NOT_REALISTIC_CLAY_CERTIFICATION',presentation:'CHANDELLE_LOOPER_TOY_GRAVITY_ONLY',modelBoundary:'TOY_GRAVITY_ONLY',gravity_mps2,gravityProvenance:'P2_SPEC_STANDARD_GRAVITY',targetInitial_W:Object.freeze([...targetInitial_W]),targetInitialVelocity_W:Object.freeze([...targetInitialVelocity_W]),shotOrigin_W:Object.freeze([...shotOrigin_W]),cameraOrigin_W:Object.freeze([...cameraOrigin_W]),R_CW,shotTime_s,apexTime_s,gunStrategy:P8B_ENGINEERING_GUN_STRATEGY,sourceMethodReference:Object.freeze({id:null,role:'NONE_SELECTED',kinematicsStatus:'HOLD_NO_AUTHORISED_APPLICABILITY_MAPPING',thresholdEventsStatus:'HOLD_UNLESS_AUTHORISED_PREDICATE'}),provider,inputProvenance:Object.freeze({launchState:'ENGINEERING_TEST_INPUT_NOT_REAL_CLAY_MEASUREMENT',gravity:'VERIFIED_FACT_P2_SPEC_STANDARD_GRAVITY'}),limitations:Object.freeze(['gravity-only point-target flight; no aerodynamic drag/lift/moment/spin model','launch state is an engineering test input, not a measured clay throw','TEST_ONLY constant-speed pellet provider with straight pellet path','no dense shot-cloud model','engineering intercept-reference gun strategy is NOT a coaching method','no source coaching method is asserted for this proof'])});
}
...
export function looperTargetStateAt(scenario,t_s){
  ...
  const phaseEps=64*Number.EPSILON*Math.max(1,Math.abs(scenario.targetInitialVelocity_W[2]),Math.abs(scenario.gravity_mps2*t_s));
...
function interceptAt(scenario,t_s){
  return interceptWithTargetTrajectoryProvider({provider:scenario.provider,shotOrigin_W:scenario.shotOrigin_W,targetPositionAtTau_W:tau=>looperTargetStateAt(scenario,t_s+tau).position_W,maxTau_s:2,tolerance_s:1e-10,maxIterations:140});
}
...
export function simulateCanonicalLooperGravity(scenario,t_s,{rateDt_s=1e-4}={}){
  ...
  const targetRelativeCamera_W=sub3(targetState.position_W,scenario.cameraOrigin_W),targetRange_m=norm3(targetRelativeCamera_W);
  ...
  const targetAngles=apparentAnglesFromCameraVector(targetVector_C),targetLosOmega_W=losAngularVelocityVector(targetRelativeCamera_W,targetState.velocity_W);
  const currentIntercept=interceptAt(scenario,t_s);if(!currentIntercept.valid)throw new Error(`P8B intercept invalid: ${currentIntercept.reason}`);
  ...
  const t0=Math.max(0,t_s-rateDt_s),prevIntercept=interceptAt(scenario,t0);if(!prevIntercept.valid)throw new Error(`P8B previous intercept invalid: ${prevIntercept.reason}`);
  ...
  const shotIntercept=interceptAt(scenario,scenario.shotTime_s);if(!shotIntercept.valid)throw new Error(`P8B shot intercept invalid: ${shotIntercept.reason}`);
