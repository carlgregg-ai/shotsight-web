...
for(const id of ['playPause','stepBack','stepForward','speed','scrub','telemetry','qaRows','certLock','holds'])assert.match(html,new RegExp(`id=["']${id}["']`),`missing browser control ${id}`);
for(const required of ['ENGINEERING PROOF · NOT REALISTIC CLAY CERTIFICATION · NOT INSTRUCTIONAL','ACQUISITION · CONNECTION · SPEED_MATCH = HOLD','createFlatCrosserDebugSession','debugFrame','scrubNormalized'])assert.ok(html.includes(required),`missing P7 browser invariant: ${required}`);
...
console.log(JSON.stringify({suite:'ShotSight P7 browser debug contract v1',status:'PASS',tests:{controlsPresent:true,certificationLock:true,holdRendering:true,sharedClock:true,testOnlyProvider:true,scrubDirect:true}},null,2));
