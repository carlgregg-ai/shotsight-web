...
//
// IMPORTANT: this is NOT realistic clay-flight certification and NOT production
// instructional motion. It exists to prove that target state, ballistic TOF/intercept,
// projection, bore state, method provenance and narrative events can share one clock.

import {add3,sub3,scale3,norm3,assertFiniteNumber,assertFiniteVec3} from './target-engine-v1.mjs';
...
export function createTestOnlyConstantSpeedProvider(speed_mps=400){
  ...
  return Object.freeze({
    id:'P7_TEST_ONLY_CONSTANT_SPEED',status:'TEST_ONLY_NOT_INSTRUCTIONAL',speed_mps,
...
export const P7_DEFAULT_CAMERA_R_CW=Object.freeze([
  Object.freeze([1,0,0]),
  Object.freeze([0,0,-1]),
  Object.freeze([0,1,0])
]);

export function createCanonicalFlatCrosserScenario({
  targetInitial_W=[-6,35,1.5],targetVelocity_W=[15,0,0],shotOrigin_W=[0,0,1.5],cameraOrigin_W=[0,0,1.65],
  R_CW=P7_DEFAULT_CAMERA_R_CW,shotTime_s=0.8,methodRegistryId='NSCA_LONG_CROSSER_PULL_AWAY',provider=createTestOnlyConstantSpeedProvider()
}={}){
  ...
  return Object.freeze({
    id:'P7_FLAT_CROSSER_ENGINEERING_PROOF_V1',status:'ENGINEERING_PROOF_NOT_REALISTIC_CLAY_CERTIFICATION',
    targetInitial_W:Object.freeze([...targetInitial_W]),targetVelocity_W:Object.freeze([...targetVelocity_W]),
    shotOrigin_W:Object.freeze([...shotOrigin_W]),cameraOrigin_W:Object.freeze([...cameraOrigin_W]),R_CW,
    shotTime_s,methodEntry,provider,
    limitations:Object.freeze(['constant-velocity target','TEST_ONLY constant-speed pellet provider','straight pellet path','no realistic clay aerodynamics','no dense shot-cloud model','method provenance attached but threshold-dependent coaching states remain held'])
  });
}
...
function interceptAt(scenario,t_s){
  return interceptWithTimeToRangeProvider({
    provider:scenario.provider,shotOrigin_W:scenario.shotOrigin_W,
    targetPosition_W:targetPositionAt(scenario,t_s),targetVelocity_W:scenario.targetVelocity_W,
    maxTau_s:2,tolerance_s:1e-10,maxIterations:120
  });
}
...
export function simulateCanonicalFlatCrosser(scenario,t_s,{rateDt_s=1e-4}={}){
  ...
  if(!currentIntercept.valid)throw new Error(`P7 intercept invalid: ${currentIntercept.reason}`);
  ...
  if(!prevIntercept.valid)throw new Error(`P7 previous intercept invalid: ${prevIntercept.reason}`);
  ...
  if(!shotIntercept.valid)throw new Error(`P7 shot intercept invalid: ${shotIntercept.reason}`);
  ...
  return Object.freeze({
    scenarioId:scenario.id,scenarioStatus:scenario.status,t_s,
    target:Object.freeze({position_W:targetPosition_W,velocity_W:scenario.targetVelocity_W,speed_mps:norm3(scenario.targetVelocity_W),range_m:targetRange_m,az_rad:targetAngles.az_rad,el_rad:targetAngles.el_rad,losAngularVelocity_W:targetLosOmega_W}),
    gun:Object.freeze({bore_W:currentIntercept.bore_W,az_rad:boreAngles.az_rad,el_rad:boreAngles.el_rad,angularSpeed_radps:gunRate.angularSpeed_radps}),
    relationship:Object.freeze({signedApparentAzSeparation_rad:boreAngles.az_rad-targetAngles.az_rad,physicalLead_m:currentIntercept.physicalLead_m,apparentLeadAngle_rad:currentIntercept.apparentLeadAngle_rad}),
    ballistic:Object.freeze({providerId:scenario.provider.id,providerStatus:scenario.provider.status,currentIntercept,shotIntercept,pelletArrival_s:scenario.shotTime_s+shotIntercept.pelletTOF_s}),
    method:Object.freeze({id:scenario.methodEntry.contract.id,evidenceClass:scenario.methodEntry.contract.evidenceClass,sources:scenario.methodEntry.contract.sources,thresholdEventsStatus:'HOLD_UNLESS_AUTHORISED_PREDICATE'}),
    narrative,
    ...
    certification:Object.freeze({realisticClay:false,instructionalMotion:false,engineeringIntegrationProof:true})
  });
}
