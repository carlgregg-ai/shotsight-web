...
const motionLessons = new Set(['flat_long_crosser','low_fast_incomer_cutoff','chandelle_apex','rising_teal_under_power','crossing_rabbit','driven_incoming','rising_quartering_outgoing','fast_flat_quartering_away']);
...
async function run(label, viewport) {
  ...
  try {
    ...
    const lessonCount = await page.locator('.playbook-card').count();if (lessonCount !== 10) throw new Error(`${label}: expected 10 certified lessons after Stage 8 batch 2, got ${lessonCount}`);
    ...
    for (let i = 0; i < lessonCount; i++) {
      ...
      if(id==='rising_quartering_outgoing'||id==='fast_flat_quartering_away'){const txt=await page.locator('#playbookLessonBody').innerText();const source=id==='rising_quartering_outgoing'?'NSCA_RISING_OUTGOING':'NSCA_ALWAYS_BEHIND';if(!txt.includes(source))throw new Error(`${label}: ${id} direct source attribution missing`);if(!/SHOTSIGHT_HYPOTHESIS/.test(txt))throw new Error(`${label}: ${id} diagnostic hypothesis label missing`);if(!/Do not/.test(txt))throw new Error(`${label}: ${id} hold guardrail missing`)}
    ...
    const search=page.locator('#playbookSearch');await search.fill('rising quartering outgoing');await page.locator('#playbookIntent:not(.diagnose)').waitFor({state:'visible'});if(await page.locator('.playbook-card[data-playbook-id="rising_quartering_outgoing"]').count()!==1)throw new Error(`${label}: rising-quarter expansion search failed`);await search.fill('fast flat quarterer');if(await page.locator('.playbook-card[data-playbook-id="fast_flat_quartering_away"]').count()!==1)throw new Error(`${label}: fast-quarter expansion search failed`);
    ...
    console.log(`PASS ${label}: ${lessonCount} certified lessons, ten schematics, eight source-safe motions, two explicit holds, Stage 8 expansion search/evidence and existing routing verified.`);
