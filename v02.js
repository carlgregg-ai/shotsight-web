...
function ensurePlaybookAssets(){
 if(!document.querySelector('link[data-playbook-css]')){const l=document.createElement('link');l.rel='stylesheet';l.href='playbook.css?v=20260904-0435';l.dataset.playbookCss='1';document.head.appendChild(l)}
 if(!document.querySelector('script[data-playbook-js]')){const s=document.createElement('script');s.src='playbook.js?v=20260904-0435';s.dataset.playbookJs='1';s.onload=ensurePlaybookMotion;document.body.appendChild(s)}else ensurePlaybookMotion();
}
function ensurePlaybookMotion(){
 ...
 const s=document.createElement('script');s.src='playbook-motion.js?v=20260904-0435';s.dataset.playbookMotion='1';document.body.appendChild(s);
}
function refineCopy(){
 const hero=document.querySelector('#diagnoseView .diagnose-hero');if(hero){const p=hero.querySelector('p');if(p)p.textContent='Start with the target presentation, then the repeatable result and what you saw or felt. ShotSight will test a coaching hypothesis and keep uncertainty visible when the evidence does not support a fix.';const badge=hero.querySelector('.diag-badge');if(badge)badge.textContent='PRESENTATION → RESULT → TEST → FIX'}
 const quick=document.querySelector('.quick-card[data-go="diagnoseView"] small');if(quick)quick.textContent='Classify the target, test the miss, and keep competing causes open.';
}
