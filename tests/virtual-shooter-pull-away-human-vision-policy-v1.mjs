...
assert.ok(separating.command.desiredAz_rad>r.az_rad);

// Once separation begins, leaving the original connection tolerance must not command a return
// to the clay. This phase persistence is learner-internal method state, not oracle information.
const midAhead=.024;
const midGun=createGunPlantState({t_s:r.observationTime_s,az_rad:r.az_rad+midAhead,el_rad:r.el_rad,azRate_radps:r.apparentAzRate_radps,elRate_radps:r.apparentElRate_radps});
const persisted=createPullAwayPolicyStateV1({visualEvidence:tracked,gunState:midGun,separationTarget_rad:.035,previousPhase:separating.phase});
assert.equal(persisted.phase,'DEVELOP_SEPARATION');
assert.equal(persisted.trigger,false);
assert.ok(persisted.command.desiredAz_rad>r.az_rad);
...
const ready=createPullAwayPolicyStateV1({visualEvidence:tracked,gunState:triggerGun,separationTarget_rad:.035,previousPhase:persisted.phase});
assert.equal(ready.phase,'TRIGGER_READY');assert.equal(ready.trigger,true);

// Visual loss overrides phase persistence: no hidden extrapolated pull-away is allowed.
const lostHistory=[...history,obs(.30,{visible:false})];
const lost=buildEllisHumanVisualEvidence(lostHistory,{occluded:true,contrast:1,clutter:0});
const lostPolicy=createPullAwayPolicyStateV1({visualEvidence:lost,gunState:triggerGun,separationTarget_rad:.035,previousPhase:'DEVELOP_SEPARATION'});
assert.equal(lostPolicy.phase,'WAIT_FOR_ACQUISITION');assert.equal(lostPolicy.trigger,false);
...
console.log(JSON.stringify({status:'PASS',schema:'PULL_AWAY_HUMAN_VISION_POLICY_GATE_V1',rawObservationAccepted:false,earlyPreciseAim:false,trackedPolicyPhase:separating.phase,persistentSeparationPhase:persisted.phase,triggerPolicyPhase:ready.phase,visualLossResetsPhase:true,acquisitionExperience:true,antiCheat:'PASS'},null,2));
