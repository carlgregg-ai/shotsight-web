...
function renderShooterResult(){
  ...
  document.querySelector('#diagnosisDemo')?.addEventListener('click',()=>{closeShooterDiagnosis();const idx=window.shotDemos?.findIndex?.(d=>d.id===r.demo)??-1;if(idx>=0&&typeof window.openDemo==='function')window.openDemo(idx)});
  document.querySelector('#diagnosisDrill')?.addEventListener('click',()=>{closeShooterDiagnosis();const a=window.activities?.find?.(x=>x.id===r.drill);if(a&&typeof window.requestOpen==='function')window.requestOpen(a)});
}
